...
class image_converter:
  ...
  def blob_forward_kinematics(self, image):
      return (self.detect_red(image), self.detect_joint_angles(image))

  def chamfer_forward_kinematics(self, image):
      ja_array = self.detect_joint_angles_chamfer(image)

      A_1_0 = np.append(self.rotation_matrix_around_z(ja_array[0]), self.displacement_vector(0, 0, 0), axis = 1)
      A_1_0 = np.append(A_1_0, np.array([[0, 0, 0, 1]]), axis = 0)

      A_2_1 = np.append(self.rotation_matrix_around_z(ja_array[0]), self.displacement_vector(3, 0, 0), axis=1)
      A_2_1 = np.append(A_2_1, np.array([[0, 0, 0, 1]]), axis=0)

      A_3_2 = np.append(self.rotation_matrix_around_z(ja_array[0]), self.displacement_vector(3, 0, 0), axis=1)
      A_3_2 = np.append(A_3_2, np.array([[0, 0, 0, 1]]), axis=0)

      A_4_3 = np.append(np.identity(3), self.displacement_vector(3, 0, 0), axis=1)
      A_4_3 = np.append(A_4_3, np.array([[0, 0, 0, 1]]), axis=0)

      A_2_0 = np.matmul(A_1_0, A_2_1)
      A_3_0 = np.matmul(A_2_0, A_3_2)
      A_4_0 = np.matmul(A_3_0, A_4_3)

      pos_array = A_4_0[:,3]
      pos_array = np.delete(pos_array, (3), axis = 0)

      return(ja_array, pos_array)

  def rotation_matrix_around_z(self, theta):
      return np.array([[np.cos(theta), -1 * np.sin(theta), 0],
                       [np.sin(theta), np.cos(theta), 0],
                       [0, 0, 1]])

  def displacement_vector(self, x, y, z):
      return np.array([[x],
                       [y],
                       [z]])

  def open_loop_using_ik(self, image):

      ja = self.detect_joint_angles_chamfer(image)
      jacobian = np.array([
          [3*np.cos(ja[0]) + 3*np.cos(ja[0] + ja[1]) + 3*np.cos(ja[0] + ja[1] + ja[2]),
           3*np.cos(ja[0] + ja[1]) + 3*np.cos(ja[0] +ja[1] + ja[2]),
           3*np.cos(ja[0] + ja[1] + ja[2])],
          [-3*np.sin(ja[0]) + -3*np.sin(ja[0] + ja[1]) + -3*np.sin(ja[0] + ja[1] + ja[2]),
           -3*np.sin(ja[0] + ja[1]) + -3*np.sin(ja[0] +ja[1] + ja[2]),
           -3*np.sin(ja[0] + ja[1] + ja[2])]
      ])
      ...
      q_d = ja + (dt * np.dot(J_inv, self.error_d.transpose()))
      return q_d
  ...
  def callback(self,data):
    ...
    cv2.waitKey(3)
    self.end_effector.publish(self.chamfer_forward_kinematics)

    #print("Joint Angles")
    #print(self.chamfer_forward_kinematics(cv_image)[0])
    #print("Joint Coordinates")
    #print(self.chamfer_forward_kinematics(cv_image)[1])
