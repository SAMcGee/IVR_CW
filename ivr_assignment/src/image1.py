...
class image_converter:
  ...
  def __init__(self):
    ...
    # Initialise previous time value
    ...
    # Desired joint position publisher
    self.robot_joint1_pub = rospy.Publisher("/robot/joint1_position_controller/command", Float64, queue_size=10)
    self.robot_joint2_pub = rospy.Publisher("/robot/joint2_position_controller/command", Float64, queue_size=10) 
    self.robot_joint3_pub = rospy.Publisher("/robot/joint3_position_controller/command", Float64, queue_size=10) 
    self.robot_joint4_pub = rospy.Publisher("/robot/joint4_position_controller/command", Float64, queue_size=10)
    
    # Forward Kinematics position estimation publisher
    ...
    # Target estimated position subscriber
    ...
    # Rectangular box estimated position subscriber
  ...
  def callback1(self,data):
    ...
    try: 
      ...
      #Publish joints only when desired joint positions are set
      if q_d is not None:
        self.robot_joint1_pub.publish(self.joint1)
        self.robot_joint2_pub.publish(self.joint2)
        self.robot_joint3_pub.publish(self.joint3)
        self.robot_joint4_pub.publish(self.joint4)
      #Publish FK end effector estimation results
  ...
  def joint_callback(self, data):
    #Set previous joint positions before updating current joint data
  ...
  def forward_kinematics(self):
    #Rotation around Z in FK calculations
  ...
  def null_space(self):
    ...
    cur_time = np.array([rospy.get_time()])
    #Change in time from previous step to current step
    dt = cur_time - self.time_previous_step
    ...
    # Set desired position as position of the target
    pos_d = np.array([self.target_x.data, self.target_y.data, self.target_z.data])
    ...
    # Get previous joint angles
    q_prev = np.array([self.previous_joint1.data, self.previous_joint2.data, self.previous_joint3.data, self.previous_joint4.data])
    
    # Difference in joint angles from last joint angles
    q_diff = q - q_prev
    
    # Previous cost function, euclidean distance between previous position and box position
    wprev = np.linalg.norm(prev_pos - rectangle_pos)
    
    # Previous cost function, euclidean distance between previous position and box position    
    w = np.linalg.norm(pos - rectangle_pos)
    
    #Difference between current distance and previous distance from box
    w_diff = w - wprev
    
    # dq0 is array of partial derivatives of the cost with respect to the joint angles.
    # We want to move positively along the gradient to maximise the distance between the robot end effector and the box while moving towards the target. 
    ...
    #derivative of desired joint angles
  ...
  def calculate_end_effector_coords(self,image1, image2):
    ...
    # Detect yellow joint position to use as [0,0,0] coordinate
    im1center = a * self.detect_color(image1, "yellow")
    im2center = a * self.detect_color(image2, "yellow")
    
    # Detect red joint as end effector
    ...
    # Convert red coords to space with yellow joint as [0,0,0]
